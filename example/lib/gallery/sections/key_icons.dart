...
class KeyIconsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text('Key Icons', style: theme.textTheme.displayMedium),
        const SizedBox(height: 16),
        Wrap(
          children: [
            const NesKeyIcon(buttonKey: 'a'),
            const SizedBox(width: 16),
            const NesKeyIcon(buttonKey: 'a', pressed: true),
            const SizedBox(width: 16),
            _Toogler(),
          ],
        ),
...
class _Toogler extends Phased<bool> {
  ...
  @override
  Widget build(BuildContext context) {
    return NesKeyIcon(buttonKey: 'a', pressed: state.value);
  }
}
